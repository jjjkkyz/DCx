...
class Dataset:
    def __init__(self, dataname,base_exp_dir):
        super(Dataset, self).__init__()
        print('Load data: Begin')
        self.device = torch.device('cuda')

        self.result_dir = base_exp_dir
        self.data_name = dataname + '.npz'

        if os.path.exists(os.path.join(self.result_dir, self.data_name)):
            print('Query data existing. Loading data...')
        else:
            print('Query data not found. Processing data...')
            process_data(self.result_dir, dataname)

        load_data = np.load(os.path.join(self.result_dir, self.data_name))
